...
impl<Label, Initiator, Instruction, Terminator> GraphOO<Label, Initiator, Instruction, Terminator> {
    ...
    {
        match self {
            GraphOO::Single(block) => block.and_then_into(bind_instruction),
            GraphOO::Many {
                entry,
                labels,
                exit_label,
                ...
                let entry_graph = entry.and_then_into(bind_instruction, bind_terminator);
                let entry = entry_graph.entry;
                let mut labels =
                    labels.and_then_into(bind_initiator, bind_instruction, bind_terminator);
                let exit_graph = exit.and_then_into(
                    |initiator| bind_initiator(exit_label, initiator),
                    bind_instruction,
                );
                labels.map.extend(entry_graph.labels.map);
...
impl<Label, Initiator, Instruction, Terminator> GraphCO<Label, Initiator, Instruction, Terminator> {
        ...
        BindTerminator: Fn(&Terminator) -> GraphOC<Label2, Initiator2, Instruction2, Terminator2>,
    {
        ...
        let exit_graph = self.exit.and_then(
            |initiator| bind_initiator(&self.exit_label, initiator),
            bind_instruction,
        );
        ...
        BindTerminator: Fn(Terminator) -> GraphOC<Label2, Initiator2, Instruction2, Terminator2>,
    {
        let labels = self
            .labels
            .and_then_into(bind_initiator, bind_instruction, bind_terminator);
        let exit_label = self.exit_label;
        let exit_graph = self.exit.and_then_into(
            |initiator| bind_initiator(exit_label, initiator),
            bind_instruction,
        );
        GraphCC { labels } + exit_graph
...
impl<Label, Initiator, Instruction, Terminator> GraphOC<Label, Initiator, Instruction, Terminator> {
    ...
    where
        ...
        BindTerminator: Fn(Terminator) -> GraphOC<Label2, Initiator2, Instruction2, Terminator2>,
    {
        let entry_graph = self.entry.and_then_into(bind_instruction, bind_terminator);
        let entry = entry_graph.entry;
        let mut labels =
            self.labels
                .and_then_into(bind_initiator, bind_instruction, bind_terminator);
...
impl<Label, Initiator, Instruction, Terminator> Labels<Label, Initiator, Instruction, Terminator> {
    ...
    {
        let mut result = Labels::new();
        for (label, block) in self.map.into_iter() {
            let graph = block.and_then_into(
                |initiator| bind_initiator(label, initiator),
                bind_instruction,
                bind_terminator,
            );
